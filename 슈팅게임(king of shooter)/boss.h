...
public:
	HRESULT init(string p_imgUnitKey, string p_imgBulletKey, int p_posX, int p_posY, int p_sizeX, int p_sizeY,
		int p_hp, int p_speed, int p_maxFrameX, int p_maxFrameY, int fireRate, int p_waitTime,
		player *p_player, bool p_isAimTarget);
	...
	//float AutoFireAttack();	//막갈기기
	void animation() override;	//virtual 해서 자식클래스를 객체로 해서 오버라이드 
								//하면 걔는 또 따로 움직이게 할 수 잇음.
